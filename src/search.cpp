...
std::string SearchManager::pv(const Search::Worker&     worker,
    ...
    for (size_t i = 0; i < multiPV; ++i)
    {
        ...
        //NOTE: The WASM engines occasionally throw a divide by zero error. Not sure why this is happening because +1 is added above.
        ss << " nodes " << nodes << " nps " << nodes * 1000 / (time != 0 ? time : 1) << " hashfull " << tt.hashfull()
#ifndef __NO_SYZYGY__
           << " tbhits " << tbHits
#endif
           << " time " << time << " pv";
